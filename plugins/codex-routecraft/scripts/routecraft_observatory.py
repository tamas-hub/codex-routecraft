...
def build_payload(alias: str|None=None) -> dict:
    ...
    mem=memory_status(source,memory,os.environ.copy())
    return {
      "schema_version":1,
...
def send(endpoint: str, token: str, payload: dict) -> dict:
    ...
    req=urllib.request.Request(endpoint,data=body,headers={"Content-Type":"application/json","Authorization":"Bearer "+token,"User-Agent":"RouteCraft-Observatory/1"},method="POST")
