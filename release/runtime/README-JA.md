# RouteCraft Runtime 0.7.3 スターター

このパッケージは、RouteCraft Local Runtime 0.7.3をWindowsまたはmacOSへ導入するための小さなスターターです。ソースを同梱スクリプトから直接実行するのではなく、公式Repositoryからrelease tagを取得し、固定commitと一致した場合だけRepository verifierと公式setup scriptを実行します。
...
```powershell
Get-FileHash .\routecraft-runtime-0.7.3-windows.zip -Algorithm SHA256
...
```powershell
Unblock-File .\routecraft-runtime-0.7.3-windows\install-routecraft.ps1
```
...
```sh
shasum -a 256 routecraft-runtime-0.7.3-macos.zip
cat SHA256SUMS.txt
```
