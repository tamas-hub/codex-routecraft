...
## RouteCraft Local Runtime 0.7.3
...
> Status: v0.7.3 observe-first. Codex multi-agent APIs are evolving; RouteCraft is capability-aware and fails conservatively when a requested model/effort lane or benchmark isolation boundary cannot be selected and verified.
...
Use the signed-off Windows or macOS starter ZIP when installing a second device: it verifies the release checkout, runs a read-only plan, and requires an explicit `INSTALL` confirmation for the local transaction. A mutable `main` checkout is for development and unreleased testing only.
