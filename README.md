...
The bundled `routecraft_telemetry.py` collector maps the model and reasoning effort selected for the human-started parent task to the model and effort that actually ran in each child. It records only pseudonymous run linkage, role, timing, and token counters; prompts, workspace paths, filenames, task paths, and raw session IDs are excluded.
